...
export const AdminAirdrop: React.FC = () => {
    ...
    const handlePauseMission = async () => {
        ...
        const result = await Swal.fire({
            ...
            text: missionPaused
                ? 'Resume the mission and allow users to claim $BAG again?'
                : 'Pause the mission? All $BAG claims will be disabled until resumed.',
            icon: 'warning',
            showCancelButton: true,
    ...
    const handleExportData = async () => {
        try {
            ...
            link.click();
            link.remove();
            Swal.fire({ title: 'EXPORT COMPLETE', text: 'Member $BAG snapshot downloaded.', icon: 'success', background: '#0a0a0a', color: '#fff' });
        } catch (error) {
    ...
    const handleFullWipe = async () => {
        const result = await Swal.fire({
            ...
            html: '<p style="color:#ccc">This permanently deletes all $BAG, tasks, campaigns and resets every member. User accounts are preserved but all mission progress is erased.</p><p style="color:#fcd535; margin-top:12px"><strong>Download the export first before proceeding.</strong></p>',
            icon: 'error',
            showCancelButton: true,
            confirmButtonColor: '#d33',
    ...
    const triggerSnapshot = async () => {
        const result = await Swal.fire({
            ...
            text: "Award 2,000 $BAG bonus to the Top 100 recruiters now?",
            icon: 'info',
            showCancelButton: true,
            confirmButtonColor: '#fcd535',
    ...
    const handleGrantBonus = async (userId: string, currentWallet: string) => {
        ...
        if (bonusAmount) {
            try {
                const res = await api.post('/api/airdrop/admin/bonus-xp', { userId, bonusTokens: parseInt(bonusAmount) });
                Swal.fire({
    ...
    const exportCSV = () => {
        let csvContent = "data:text/csv;charset=utf-8,Wallet,BAG,Referrals,Tier\n";
                        ...
                        {showTaskForm && (
                            <form onSubmit={handleCreateTask} className="bg-black/40 border border-alphabag-yellow/50 rounded-2xl p-8 space-y-6 animate-in slide-in-from-top-4 duration-500">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-alphabag-muted uppercase tracking-widest pl-1">$BAG Reward</label>
                                ...
                                <div key={t.id} className="p-6 rounded-2xl bg-white/[0.02] border border-white/5 flex flex-col md:flex-row justify-between items-center gap-6 group hover:border-white/20 transition-all">
                                    ...
                                    <div className="flex items-center gap-10">
                                        <div className="text-right">
                                            ...
                                            <div className="text-xl font-black text-white">{t.rewardTokens} <span className="text-[10px] text-alphabag-yellow tracking-tighter">$BAG</span></div>
                                        </div>
                            ...
                            <table className="w-full text-left">
                                <thead className="bg-white/5 text-[9px] uppercase text-alphabag-muted font-black tracking-widest">
                                    <tr>
                                        ...
                                        <th className="p-6 text-right">Power (BAG)</th>
            ...
            <div className="bg-[#0a0a0a] border border-alphabag-yellow/20 rounded-2xl p-8 space-y-6">
                ...
                <div className="flex flex-col md:flex-row gap-4 items-end">
                    ...
                    <Button
                        ...
                        className="bg-alphabag-yellow text-black font-black uppercase tracking-widest text-[11px] px-8 py-4 rounded-xl hover:shadow-[0_0_20px_rgba(252,213,53,0.3)] transition-all whitespace-nowrap"
                    >
