...
export const Airdrop: React.FC = () => {
    ...
    const getXPTier = (xp: number) => {
        if (xp >= 2500) return { name: 'SYNDICATE', next: null, current: 2500, min: 2500, color: 'text-alphabag-yellow', border: 'border-alphabag-yellow/40', bg: 'bg-alphabag-yellow/10', glow: 'shadow-[0_0_20px_rgba(252,213,53,0.2)]' };
        if (xp >= 1000) return { name: 'ELITE', next: 2500, current: xp, min: 1000, color: 'text-purple-400', border: 'border-purple-500/40', bg: 'bg-purple-500/10', glow: 'shadow-[0_0_20px_rgba(168,85,247,0.1)]' };
        if (xp >= 500)  return { name: 'AGENT', next: 1000, current: xp, min: 500, color: 'text-blue-400', border: 'border-blue-500/40', bg: 'bg-blue-500/10', glow: '' };
        if (xp >= 100)  return { name: 'OPERATIVE', next: 500, current: xp, min: 100, color: 'text-green-400', border: 'border-green-500/40', bg: 'bg-green-500/10', glow: '' };
        return { name: 'RECRUIT', next: 100, current: xp, min: 0, color: 'text-zinc-400', border: 'border-white/10', bg: 'bg-white/5', glow: '' };
    };
    ...
    const handleCompleteTask = async (taskId: string, requiresLink?: boolean) => {
        ...
        try {
            ...
            if (res.data.success) {
                ...
                // Refresh tasks and stats in-place (no jarring page reload)
                ...
                // Real-time balance and level sync
                await refreshUser();
            }
        } catch (err: any) {
             Swal.fire({
        ...
        <div className="max-w-7xl mx-auto space-y-8 pb-20 px-4 md:px-8">
            ...
            {missionPaused && (
                <div className="flex items-center gap-4 p-5 bg-yellow-500/10 border border-yellow-500/30 rounded-2xl animate-in slide-in-from-top-4 duration-500">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-alphabag-subtext mt-0.5">The Alpha Mission is currently paused by the core team. All XP claims are disabled. Check back soon — we'll be back online shortly.</p>
                    </div>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {(() => {
                    ...
                    const progress = Math.min(100, Math.round((myTeamSize / 1000) * 100));
                        ...
                        <div className="glass-panel p-6 bg-gradient-to-br from-white/[0.03] to-transparent border border-white/10 hover:border-alphabag-yellow/30 transition-all group relative overflow-hidden rounded-2xl">
                            ...
                            <div className="mt-4 space-y-2">
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="text-[9px] text-alphabag-muted font-bold">{1000 - myTeamSize} slots left</span>
                                </div>
                                <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
                                    <div className="h-full rounded-full transition-all duration-700 bg-alphabag-yellow" style={{width: `${progress}%`}}></div>
                                </div>
                                <div className="text-[9px] text-alphabag-muted font-bold">{myTeamSize} / 1000 MAX</div>
                            </div>
                        </div>
                    );
                })()}
                ...
                {(() => {
                    const xp = user?.airdropPoints || 0;
                    const tier = getXPTier(xp);
                    return (
                        <div className={`glass-panel p-6 bg-gradient-to-br from-white/[0.03] to-transparent border transition-all group relative overflow-hidden rounded-2xl ${tier.border} ${tier.glow}`}>
                            ...
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-1 rounded-full border ${tier.bg} ${tier.color} ${tier.border}`}>{tier.name}</span>
                            </div>
                            <div className="text-4xl font-black text-white tracking-tight">{xp.toLocaleString()} <span className="text-lg text-alphabag-muted font-bold">XP</span></div>
                                <div className="mt-4 text-[10px] text-alphabag-muted font-bold uppercase tracking-tight border-t border-white/5 pt-3">Convertible to our utility token at TGE</div>
                        </div>
                    );
                })()}
...
{(() => {
    const targetDate = stats?.tgeDate ? new Date(stats.tgeDate) : new Date(Date.now() + 30*24*60*60*1000);
    const diff = Math.max(0, targetDate.getTime() - Date.now());
    const days = Math.ceil(diff / (1000 * 60 * 60 * 24));
    return (
        <div className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-alphabag-green to-emerald-400 tracking-tight">
            {days} DAYS
        </div>
    );
})()}
            ...
            <div className="glass-panel p-6 bg-[#0a0a0a] border border-white/5 rounded-3xl relative overflow-hidden mb-8">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 ">
                        <div>
                            ...
                            <p className="text-xs text-alphabag-subtext mt-1">Recruit new members and earn <span className="text-alphabag-yellow font-bold">50 XP</span> per successful sync.</p>
                        </div>
                            ...
                            <div className="flex items-center gap-2 w-full md:w-auto">
                                ...
                                <button
                                    onClick={() => {
                                        ...
                                        const tweet = encodeURIComponent(`🚀 Join me on AlphaBAG — earn points, complete missions & be rewarded at TGE!\n\nSign up here 👇\n${refUrl}\n\n#AlphaBAG #BAG #Crypto`);
                            ...
                            return (
                                <div key={task.id} className={`flex flex-col justify-between h-full p-6 rounded-2xl border transition-all group relative overflow-hidden ${isCompleted ? 'bg-alphabag-green/5 border-alphabag-green/20' : 'bg-white/[0.02] border-white/5 hover:border-alphabag-yellow/30'}`}>
                                    ...
                                    {!isCompleted && (
                                        ...
                                        >
                                            {missionPaused ? '⏸ MISSION PAUSED' : isTaskLoading ? 'Syncing...' : (task.type === 'daily' ? 'CLAIM 50 XP DAILY' : task.type === 'weekly' ? 'CLAIM 150 XP' : `Execute Mission ${task.actionUrl && !task.requiresLink ? '↗' : ''}`)}
                                        </Button>
                                    )}
                                </div>
                            );
