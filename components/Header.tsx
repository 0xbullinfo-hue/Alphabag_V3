...
export const Header: React.FC<HeaderProps> = ({ toggleSidebar, isSidebarOpen }) => {
  ...
  return (
    <>
      <WalletManager isOpen={showWalletManager} onClose={() => setShowWalletManager(false)} />
      ...
      <header className="fixed top-0 left-0 right-0 h-16 bg-alphabag-black/60 backdrop-blur-md border-b border-alphabag-border z-50 px-4 flex items-center justify-between shadow-lg">
        <div className="flex items-center">
          ...
          <nav className="hidden md:flex items-center ml-10 space-x-8">
            <Link to="/earn" className="text-[10px] font-black uppercase tracking-[0.2em] text-alphabag-yellow hover:text-white transition-colors flex items-center gap-2">
              <Zap size={14} fill="currentColor" /> Reward Hub
            </Link>
          ...
          {stats && (
            <div className="hidden lg:flex items-center space-x-6 ml-8 text-xs text-alphabag-muted border-l border-alphabag-border pl-6">
              <div className="flex flex-col">
                ...
                <span className="text-zinc-50 font-bold tabular-data">${(stats.marketCap / 1e12).toFixed(2)}T <span className="text-alphabag-green">+1.2%</span></span>
              </div>
              <div className="flex flex-col">
                <span className="uppercase tracking-wider font-bold opacity-70 text-[9px]">Gas Hub</span>
                <span className="text-zinc-50 font-bold tabular-data flex items-center"><TrendingUp size={10} className="mr-1 text-alphabag-yellow" /> 12 Gwei</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center space-x-4">
          {isAuthenticated ? (
            <div className="relative">
              ...
              {showUserMenu && (
                <div className="absolute right-0 mt-2 w-64 glass-panel  p-2 animate-fade-in z-50">
                  ...
                  <button
                    onClick={() => { setShowUserMenu(false); setShowWalletManager(true); }}
                    className="w-full text-left px-4 py-2.5 text-xs font-black text-zinc-50 hover:bg-white/5 rounded-xl transition-all duration-200 active:scale-[0.98] flex items-center mb-1 group"
                  >
